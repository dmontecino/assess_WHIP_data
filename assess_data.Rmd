...
#
##### Is the history provided for all the events?
#
#
```{r history of the events, echo=FALSE, warning=FALSE, eval=T, cache=F}

event.codes.no.history.per.country=
lapply(event.codes.per.country, function(x)
...
# if there are rows without history
if(nrow(do.call(rbind, event.codes.no.history.per.country))>0){  

event.codes.no.history.per.country=do.call(rbind, event.codes.no.history.per.country)

# spit the data by country again now including the data of country based on the project tag
# for those rows wo country
event.codes.no.history.per.country=
lapply(sort(unique(event.codes.no.history.per.country$Country)), function(x) 
  event.codes.no.history.per.country%>%filter(Country==x)%>%
  dplyr::select(Country, Event_Code)%>%
  distinct(Country, Event_Code))

# name the list
names(event.codes.no.history.per.country)=sapply(
  event.codes.no.history.per.country, function(x) unique(x$Country))
    
# order by event code
event.codes.no.history.per.country=
  lapply(event.codes.no.history.per.country, function(x)
  x[order(x[,"Event_Code"]), ])

# move to da dataset with the event codes in a single row
event.codes.no.history.per.country.df=
  lapply(event.codes.no.history.per.country, function(x)
  data.frame(Event_Codes=paste(unique(x$Event_Code), collapse = " ")))


  event.codes.no.history.per.country.df=data.frame(
    Country=names(event.codes.no.history.per.country.df), 
    Event_Codes=unlist(event.codes.no.history.per.country.df, use.names = F))

   event.codes.no.history.per.country.df=
     event.codes.no.history.per.country.df[event.codes.no.history.per.country.df$Event_Codes!="",]

   #delete rownames
  rownames(event.codes.no.history.per.country.df)=NULL 
  
       # subset to specific country
event.codes.no.history.per.country.df=
  event.codes.no.history.per.country.df%>%filter(Country==params$country.of.interest)

if(nrow(event.codes.no.history.per.country.df)>0){
 
  
# Show the results
    
    message("No. Not all the events have a history provided. These are the events without history per country.")
    
    kableExtra::kbl(event.codes.no.history.per.country.df, 
                    caption = "Event codes without history per country") %>%
      kableExtra::kable_paper(full_width = F) %>%
      kableExtra::column_spec(1, bold = T, border_right = T) %>%
      kableExtra::column_spec(2, width = "30em")
}}

if(!exists("event.codes.no.history.per.country.df") |
    exists("event.codes.no.history.per.country.df") && nrow(event.codes.no.history.per.country.df)==0){  
    
    event.codes.no.history.per.country.df=list(data.frame(Country="", Event_Code=""))
   
   rownames(event.codes.no.history.per.country.df[[1]])<-NULL
    
    message("Yes, all events have a history reported")}
...
# Data frame event no history

event.codes.no.history.per.country.df=
  do.call(rbind,event.codes.no.history.per.country)

event.codes.no.history.per.country.df$No_history="Yes"

event.codes.no.history.per.country.df=event.codes.no.history.per.country.df%>%
...
#
```{r specimen condition, echo=FALSE, warning=FALSE, eval=T, cache=FALSE}
...
if(nrow(spec.condition.not.reported.per.country.df)>0){  
  ...
  message("No, the data show that some specimens do not the condition of the animals reported. These are the Specimen codes that do not have the 'condition' field reported")
  
  kableExtra::kbl(spec.condition.not.reported.per.country.df, 
                  caption = "Specimens with unknown condition")%>%
    kableExtra::kable_paper(full_width = F) %>%
    kableExtra::column_spec(1, bold = T, border_right = T) %>%
