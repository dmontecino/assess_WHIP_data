...
dat=lapply(c(1:3), function(x)
  read_excel("/Users/DMontecino/Desktop/Data export_bevan_federko_2022-10-21.xlsx", sheet = x, col_types = "text"))
...
```{r specimen no necropsy report, no necropsy level or no necropsy reported and no sample inventory, echo=FALSE, warning=FALSE, eval=T, cache=FALSE, message=FALSE}
...
#
#### Are all samples not used in tests (yet) unique per specimen?
#
#
```{r repeated samples without tests dates, echo=FALSE, warning=FALSE, eval=F}
...
samples.no.test.but.repeated.df=
  ...
  print("The data has sample ids that are repeated for a specific specimen but that have not been used for any tests. These sample ids are used in more than one sample and should have a '.number' after the id follwoing the guidelines here https://docs.google.com/document/d/1qdrUJKN234as7Xd5E6_7QewT6l5hQ940/edit. These are the sample ids not used for any test so far but repeated for a specimen." )
...
Note: if two or more samples get the same id and they are used for the same test conducted on the same date, then there is no way to find that they are repeated except if the info in other fields is not repeated.


```{r repeated samples with and without tests dates, echo=FALSE, warning=FALSE, eval=F}
...
samples.with.test.and.wo.test.df=
  ...
  print("The data has specimens with sample ids that contain at least two tests and one of them does not have a specified disease. This means that the two rows are from independent samples and that the sample id is repeated for more than one sample." )
...
#at least two rows, both of them with disease test: 
  #same sample used to test for different diseases: this can be correct. If it is the same sample, the rows should be identical for samples-related fields, otherwise sample id is repeated.
  #same sample used to test for the same disease a least twice (TRICKY): this can be correct but I can point suspects, specially if the test level data is the same (suspect of a test witht he same source that could be actually from different source)
  
  #same sample used to test for different disease (sample id - disease combinations should be different) protocol does not matter
  # some could be the same sample (all cells identical up to test disease) this is correct
  # some could be actually two different samples with repeated code for sample (). Potentially the sample level fields would be different
  


#are samples unique? if they dont have a test at all, the sample id should not be repeated. If it does, is because the sample id is not unique for a specific sample
dat$spec %>% 
  # select(c("event_code", "specimen_code", "sample_id", "sample_source", "disease")) %>%
  filter(is.na(disease)) %>% 
  # group_by(event_code, specimen_code, sample_id)
  count(event_code, specimen_code, sample_id) %>% arrange(desc(n))


# single test and no results (was the test actually conducted?)


# two tests, same test, same source, same date, how to know it is a repeated test or it was wrongly entered twice? Provide suspicious tests?
