...
env.spec.sample.ids.in.more.one.env.spec=
  dat$env_spec %>% 
  filter(!is.na(sample_id))%>%
  distinct(event_code, environmental_specimen_code, sample_id) %>% 
  count(sample_id) %>% 
  filter(n>1)%>%
  pull(sample_id)
...
env.spec.sample.ids.in.more.one.env.spec=
  dat$env_spec %>% 
  filter(sample_id %in% env.spec.sample.ids.in.more.one.env.spec) %>% 
  dplyr::select(country, event_code, environmental_specimen_code, sample_id) %>% 
  distinct() %>% 
  arrange(sample_id)
...
env.spec.sample.ids.in.more.one.env.spec.df=
  env.spec.sample.ids.in.more.one.env.spec%>%
  group_by(country, event_code, sample_id) %>%
  dplyr::summarise(
    environmental_specimen_codes = paste(environmental_specimen_code, collapse = ", ")) %>% 
  arrange(country)  
...
env.specimen.sample.codes.with.issues=full_join(env.sample.date.collected.earlier.than.event.date, 
                                  env.sample.date.collected.earlier.than.specimen.found,
                                  by=c("country", "event_code", "environmental_specimen_code", "sample_id"))
...
env.specimen.sample.codes.with.issues=env.specimen.sample.codes.with.issues %>% arrange(country, event_code, environmental_specimen_code, sample_id)
