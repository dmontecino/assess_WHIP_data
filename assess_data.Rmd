...
years_to_consider<-2019:2100


# which sheets are present = what data was requested
# 
dat=lapply(c(1:3), function(x) read_excel("/Users/DMontecino/Downloads/WCS Data export_bevan_federko_2023-12-04.xlsx", sheet = x, col_types = "text"))

#dat=list(params$spec, params$obs, params$env_spec)

# assigna names to the list elements
names(dat)<-c("spec", "obs", "env_spec")
...
dat$obs %>% 
  filter(!grepl(paste(years_to_consider, collapse = "|"), dat$obs$observation_project_tags)) %>% 
...
if(nrow(obs.healthy.sick.dead.not.equal.observed.sum)>0){  
  ...
  kableExtra::kbl(obs.healthy.sick.dead.not.equal.observed.sum.df, 
...
dat$spec %>% 
  filter(!grepl(paste(years_to_consider, collapse = "|"), dat$spec$specimen_project_tags)) %>% 
...
#
#
```{r sample quantity, echo=FALSE, warning=FALSE, eval=T, cache=F, message=FALSE}
...
#
#### Do all tests that have a "non-pending" and "tested" result have the date when the results were received? 
#
...
test.result.received.date.not.provided=
  dat$spec %>% 
    filter(!grepl("Pending|Not", disease_inventory_test_result) & 
...
dat$env_spec %>% 
  filter(!grepl(paste(years_to_consider, collapse = "|"), dat$env_spec$environmental_specimen_project_tags)) %>% 
...
env.test.method.not.provided=
  ...
  filter(!is.na(disease) &
         is.na(disease_inventory_test_method) & 
         disease_inventory_test_result!="Not Tested") %>%
    dplyr::select(country, event_code, environmental_specimen_code, disease,  sample_source) %>%
    distinct()
...
## TESTS WITH RESULTS BUT WITHOUT THE DATE WHEN RESULTS WERE RECEIVED
#
#### Do all tests that have a "non-pending" and a "tested" result have the date when the results were received? 
#
