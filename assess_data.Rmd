...
params:
  spec: spec
  obs: obs
  country.of.interest: country.of.interest #test 2
---
...
spec=spec[spec$Event_Code!="WCS.100294",] # 
...
# number sick, dead, heaelthy, observed is numeric
cols.num <- c("observation_number_observed",
              "observation_number_healthy",
              "observation_number_sick",
              "observation_number_dead")

obs[cols.num] <- sapply(obs[cols.num],as.numeric)
...
names(data.per.country.obs)= c(countries, "NA_Country")
...
### HISTORY
#
##### Is the history of the events reported in WHIP?
#
```{r history, echo=FALSE, warning=FALSE, eval=T, cache=FALSE}

event.history.not.reported.per.country.spec=

lapply(data.per.country.spec, function(x)
  x%>%filter(is.na(History))%>%dplyr::select(Country, Event_Code, History)%>%distinct())


event.history.not.reported.per.country.obs=

lapply(data.per.country.obs, function(x)
  x%>%filter(is.na(History))%>%dplyr::select(Country, Event_Code, History)%>%distinct())


event.history.not.reported.per.country=
  lapply(sort(unique(unlist(c(unique(do.call(rbind, event.history.not.reported.per.country.spec)[,"Country"]), 
         unique(do.call(rbind, event.history.not.reported.per.country.obs)[,"Country"]))))),  #getting the unique countries in obs and spec
         
         function(x)
           
    rbind(do.call(rbind, event.history.not.reported.per.country.spec), 
          do.call(rbind, event.history.not.reported.per.country.obs))%>%filter(Country==x)) 

event.history.not.reported.per.country=do.call(rbind,event.history.not.reported.per.country)

if(!is.null(event.history.not.reported.per.country)){
  
# if there are rows with wrong latitude

if(nrow(do.call(rbind,event.history.not.reported.per.country))>0){  

event.history.not.reported.per.country=do.call(
  rbind, event.history.not.reported.per.country)%>%
  dplyr::select(Country, Event_Code)
...
# for those rows wo country
event.history.not.reported.per.country=
lapply(sort(unique(event.history.not.reported.per.country$Country)), function(x) 
  event.history.not.reported.per.country%>%filter(Country==x)%>%
  dplyr::select(Country, Event_Code)%>%
  distinct(Country, Event_Code))

# name the list
names(event.history.not.reported.per.country)=sapply(
  event.history.not.reported.per.country, function(x) unique(x$Country))
    
# order by specimen code
event.history.not.reported.per.country=
  lapply(event.history.not.reported.per.country, function(x)
  x[order(x[,"Event_Code"]), ])

# move to da dataset with the event codes in a single row
event.history.not.reported.per.country.df=
  lapply(event.history.not.reported.per.country, function(x)
  data.frame(Event_Code=paste(unique(x$Event_Code), collapse = " ")))

event.history.not.reported.per.country.df=data.frame(
    Country=names(event.history.not.reported.per.country.df), 
    Event_Code=unlist(event.history.not.reported.per.country.df, use.names = F))

   event.history.not.reported.per.country.df=
     event.history.not.reported.per.country.df[
       event.history.not.reported.per.country.df$Event_Code!="",]

   #delete rownames
  rownames(event.history.not.reported.per.country.df)=NULL 

 # subset to specific country
event.history.not.reported.per.country.df=
  event.history.not.reported.per.country.df%>%filter(Country==params$country.of.interest)

if(nrow(event.history.not.reported.per.country.df)>0){  
  
  message("No, the data show that some events do not have the history repoerted. These are the Event codes that do not have the history reported")
  
  kableExtra::kbl(event.history.not.reported.per.country.df, 
                  caption = "Events without history")%>%
    kableExtra::kable_paper(full_width = F) %>%
    kableExtra::column_spec(1, bold = T, border_right = T) %>%
    kableExtra::column_spec(2:ncol(event.history.not.reported.per.country.df), width = "30em", border_right = T)
}}}

if(!exists("event.history.not.reported.per.country.df") |
    exists("event.history.not.reported.per.country.df") && 
   nrow(event.history.not.reported.per.country.df)==0){  
  
  event.history.not.reported.per.country=list(data.frame(Country="", Event_Code=""))
   
  rownames(event.history.not.reported.per.country[[1]])<-NULL

  
  message("Yes, all events have the history reported")}
...
#data frame event codes no history
event.history.not.reported.per.country.df=
  do.call(rbind,event.history.not.reported.per.country)

event.history.not.reported.per.country.df$No_history="Yes"

event.history.not.reported.per.country.df=event.history.not.reported.per.country.df%>%
  dplyr::select(Country , Event_Code, No_history)
...
obs.codes.date.found.later.end.date.per.country=
  lapply(data.per.country.obs, function(x)
  x%>%
...
# if there are rows with wrong latitude
if(nrow(do.call(rbind, obs.codes.date.found.later.end.date.per.country))>0){  

obs.codes.date.found.later.end.date.per.country=do.call(
  rbind, obs.codes.date.found.later.end.date.per.country)%>%
  dplyr::select(Country, Observation_code)
...
# for those rows wo country
obs.codes.date.found.later.end.date.per.country=
lapply(sort(unique(obs.codes.date.found.later.end.date.per.country$Country)), function(x) 
  obs.codes.date.found.later.end.date.per.country%>%filter(Country==x)%>%
  dplyr::select(Country, Observation_code)%>%
  distinct(Country, Observation_code))

# name the list
names(obs.codes.date.found.later.end.date.per.country)=sapply(
  obs.codes.date.found.later.end.date.per.country, function(x) unique(x$Country))
    
# order by specimen code
obs.codes.date.found.later.end.date.per.country=
  lapply(obs.codes.date.found.later.end.date.per.country, function(x)
  x[order(x[,"Observation_code"]), ])

# move to da dataset with the event codes in a single row
obs.codes.date.found.later.end.date.per.country.df=
  lapply(obs.codes.date.found.later.end.date.per.country, function(x)
  data.frame(Observation_code=paste(unique(x$Observation_code), collapse = " ")))


  obs.codes.date.found.later.end.date.per.country.df=data.frame(
    Country=names(obs.codes.date.found.later.end.date.per.country.df), 
    Observation_code=unlist(obs.codes.date.found.later.end.date.per.country.df, use.names = F))

   obs.codes.date.found.later.end.date.per.country.df=
     obs.codes.date.found.later.end.date.per.country.df[
       obs.codes.date.found.later.end.date.per.country.df$Observation_code!="",]

   #delete rownames
  rownames(obs.codes.date.found.later.end.date.per.country.df)=NULL 

    # subset to specific country
obs.codes.date.found.later.end.date.per.country.df=
  obs.codes.date.found.later.end.date.per.country.df%>%filter(Country==params$country.of.interest)

if(nrow(obs.codes.date.found.later.end.date.per.country.df)>0){
  ...
  kableExtra::kbl(obs.codes.date.found.later.end.date.per.country.df, 
...
if(!exists("obs.codes.date.found.later.end.date.per.country.df") |
    exists("obs.codes.date.found.later.end.date.per.country.df") && 
   nrow(obs.codes.date.found.later.end.date.per.country.df)==0){  

   
  obs.codes.date.found.later.end.date.per.country=list(data.frame(Country="", Observation_code=""))
   
  rownames(obs.codes.date.found.later.end.date.per.country[[1]])<-NULL
 ...
 lapply(data.per.country.obs, function(x)
  x%>%
    ...
    dplyr::select(Country, Observation_code, 
         ...
         mutate(obs.healthy.sick.dead =rowSums(select(., all_of(cols.num))))%>%
 ...
 lapply(data.per.country.obs, function(x)
  x%>%
    ...
    dplyr::select(Country, Observation_code, 
         ...
         mutate(healthy.sick.dead =rowSums(select(., all_of(
           cols.num[-1])))))
...
```{r checking observation single sps, echo=FALSE, cache=FALSE, eval=T}

observation.codes.unique.sps.per.country=
  
 lapply(data.per.country.obs, function(x)
  x%>%

    dplyr::select(Country, Observation_code, observation_common_name)%>%
    dplyr::group_by(Observation_code)%>%
    dplyr::mutate(num.species=n())%>%
    filter(num.species>1)%>%
    select(Country, Observation_code)%>%
    distinct(Country, Observation_code))
            

    
if(nrow(do.call(rbind, observation.codes.unique.sps.per.country))>0){  

  observation.codes.unique.sps.per.country=
  do.call(
  rbind, observation.codes.unique.sps.per.country)%>%
  dplyr::select(Country, Observation_code)

# spit the data by country again now including the data of country based on the project tag
# for those rows wo country
observation.codes.unique.sps.per.country=
lapply(sort(unique(observation.codes.unique.sps.per.country$Country)), function(x) 
  observation.codes.unique.sps.per.country%>%filter(Country==x)%>%
  dplyr::select(Country, Observation_code)%>%
  distinct(Country, Observation_code))

# name the list
names(observation.codes.unique.sps.per.country)=sapply(
  observation.codes.unique.sps.per.country, function(x) unique(x$Country))
    
# order by specimen code
observation.codes.unique.sps.per.country=
  lapply(observation.codes.unique.sps.per.country, function(x)
  x[order(x[,"Observation_code"]), ])

# move to da dataset with the observation codes in a single row
observation.codes.unique.sps.per.country.df=
  lapply(observation.codes.unique.sps.per.country, function(x)
  data.frame(Observation_code=paste(unique(x$Observation_code), collapse = " ")))


  observation.codes.unique.sps.per.country.df=data.frame(
    Country=names(observation.codes.unique.sps.per.country.df), 
    Observation_code=unlist(observation.codes.unique.sps.per.country.df, use.names = F))

   observation.codes.unique.sps.per.country.df=
     observation.codes.unique.sps.per.country.df[
       observation.codes.unique.sps.per.country.df$Observation_code!="",]

   #delete rownames
  rownames(observation.codes.unique.sps.per.country.df)=NULL 

      # subset to specific country
observation.codes.unique.sps.per.country.df=
  observation.codes.unique.sps.per.country.df%>%filter(Country==params$country.of.interest)

if(nrow(observation.codes.unique.sps.per.country.df)>0){


  message("No. Not all observation codes matach a single species. These are the observations with with more than one obervation per country")
  
  kableExtra::kbl(observation.codes.unique.sps.per.country.df, 
                      caption = "Observations with more than one species per country") %>%
    kableExtra::kable_paper(full_width = F) %>%
    kableExtra::column_spec(1, bold = T, border_right = T) %>%
    kableExtra::column_spec(2, width = "30em")
  # print(specimen.codes.with.animal.ids.as.na.per.country)
...
if(!exists("observation.codes.unique.sps.per.country.df") |
    exists("observation.codes.unique.sps.per.country.df") && 
   nrow(observation.codes.unique.sps.per.country.df)==0){  

    
    observation.codes.unique.sps.per.country=list(data.frame(Country="", Observation_code=""))
   
  rownames(observation.codes.unique.sps.per.country[[1]])<-NULL
    
    message("Yes, all observations have a single species")}
...
### CONDITION
#
##### Is "condition" reported in WHIP?
#
```{r specimen condition found, echo=FALSE, warning=FALSE, eval=T, cache=FALSE}

spec.condition.not.reported.per.country=

lapply(data.per.country.spec, function(x)
  
# first remove the environmental specimens. This script was developed before the 
# Env specimen module existed so, I need to remove what is a environmental specimen
# because the condition of  the animal would not be provided (there is no animal involved)
  
    x%>%filter(Comments != "This is an environmental specimen" & is.na(Condition))%>%  
    dplyr::select(Country, Event_Code, Specimen_code, Condition)%>%distinct())

# if there are rows with wrong latitude

if(nrow(do.call(rbind,spec.condition.not.reported.per.country))>0){  

  
  
spec.condition.not.reported.per.country=do.call(
  rbind, spec.condition.not.reported.per.country)%>%
  dplyr::select(Country, Specimen_code)
...
# for those rows wo country
spec.condition.not.reported.per.country=
lapply(sort(unique(spec.condition.not.reported.per.country$Country)), function(x) 
  spec.condition.not.reported.per.country%>%filter(Country==x)%>%
  dplyr::select(Country, Specimen_code)%>%
  distinct(Country, Specimen_code))

# name the list
names(spec.condition.not.reported.per.country)=sapply(
  spec.condition.not.reported.per.country, function(x) unique(x$Country))
    
# order by specimen code
spec.condition.not.reported.per.country=
  lapply(spec.condition.not.reported.per.country, function(x)
  x[order(x[,"Specimen_code"]), ])

# move to da dataset with the event codes in a single row
spec.condition.not.reported.per.country.df=
  lapply(spec.condition.not.reported.per.country, function(x)
  data.frame(Specimen_code=paste(unique(x$Specimen_code), collapse = " ")))


  spec.condition.not.reported.per.country.df=data.frame(
    Country=names(spec.condition.not.reported.per.country.df), 
    Specimen_code=unlist(spec.condition.not.reported.per.country.df, use.names = F))

   spec.condition.not.reported.per.country.df=
     spec.condition.not.reported.per.country.df[
       spec.condition.not.reported.per.country.df$Specimen_code!="",]

   #delete rownames
  rownames(spec.condition.not.reported.per.country.df)=NULL 

 # subset to specific country
spec.condition.not.reported.per.country.df=
  spec.condition.not.reported.per.country.df%>%filter(Country==params$country.of.interest)

if(nrow(spec.condition.not.reported.per.country.df)>0){  
  
  message("No, the data show that some specimens do not have the condition reported. These are the Specimen codes that do not have the condition field reported")
  
  kableExtra::kbl(spec.condition.not.reported.per.country.df, 
                  caption = "Specimens without condition reported")%>%
    kableExtra::kable_paper(full_width = F) %>%
    kableExtra::column_spec(1, bold = T, border_right = T) %>%
    kableExtra::column_spec(2:ncol(spec.condition.not.reported.per.country.df), width = "30em", border_right = T)
}}

if(!exists("spec.condition.not.reported.per.country.df") |
    exists("spec.condition.not.reported.per.country.df") && 
   nrow(spec.condition.not.reported.per.country.df)==0){  
  
  spec.condition.not.reported.per.country=list(data.frame(Country="", Specimen_code=""))
   
  rownames(spec.condition.not.reported.per.country[[1]])<-NULL

  
  message("Yes, all specimens have the condition reported")}
...
#
If no necropsy will be conducted with the specimen, then we need the "no necropsy" level provided in the field 'Necropsy level". If the necropsy will be conducted then select "Pending".
...
not have anything associated with necropsy or samples")
  ...
  kableExtra::kbl(list.of.potential.observations.df, 
    ...
    kableExtra::kable_paper(full_width = F) %>%
    kableExtra::column_spec(1, bold = T, border_right = T) %>%
    kableExtra::column_spec(2:ncol(list.of.potential.observations.df), width = "30em", border_right = T)
}}
...
obs.codes.date.found.later.end.date.per.country.df=
  ...
  do.call(rbind, obs.codes.date.found.later.end.date.per.country)

obs.codes.date.found.later.end.date.per.country.df$Inconsistent_observed_end_date="Yes"
...
obs.codes.date.found.earlier.start.date.per.country$Inconsistent_observed_start_date="Yes"




obs.codes.total.obs.healthy.sick.dead.per.country.df=
  
  do.call(rbind, total.obs.healthy.sick.dead.per.country)

obs.codes.total.obs.healthy.sick.dead.per.country.df$No_animal_in_the_observation="Yes"




obs.codes.obs.healthy.sick.dead.equal.observed.sum.per.country.df=
 
    do.call(rbind, obs.healthy.sick.dead.equal.observed.sum.per.country)

obs.codes.obs.healthy.sick.dead.equal.observed.sum.per.country.df$Inconsistent_sum_and_total="Yes"
 

obs.codes.observation.codes.unique.sps.per.country.df=
 
    do.call(rbind, observation.codes.unique.sps.per.country)

obs.codes.observation.codes.unique.sps.per.country.df$More_than_one_sps="Yes"
...
spec.condition.not.reported.per.country.df=
    ...
    do.call(rbind, spec.condition.not.reported.per.country)

spec.condition.not.reported.per.country.df$No_condition="Yes"
...
#
```{r summmary sample source, echo=FALSE, warning=T, eval=T, cache=F, message=T}
